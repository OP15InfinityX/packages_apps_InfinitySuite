...
import com.infinity.suite.fragments.ButtonBacklightBrightness;
import com.infinity.support.preferences.LineageSystemSettingSeekBarPreference;
...
public class ButtonSettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_USE_AXION_VOLUME_DIALOG = "use_axion_volume_dialog";
    ...
    private SwitchPreferenceCompat mUseAxionVolumeDialog;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mVolumePanelLeft != null) {
            mVolumePanelLeft.setChecked(isAudioPanelOnLeft);
        }

        // Axion volume dialog
        mUseAxionVolumeDialog = (SwitchPreferenceCompat) prefScreen.findPreference(KEY_USE_AXION_VOLUME_DIALOG);
        if (mUseAxionVolumeDialog != null) {
            mUseAxionVolumeDialog.setOnPreferenceChangeListener(this);
        }

        final PreferenceCategory vibCategory = prefScreen.findPreference(KEY_VIBRATE_CATEGORY);
        if (vibCategory != null && (!TelephonyUtils.isVoiceCapable(getActivity()) || !DeviceUtils.hasVibrator(getActivity()))) {
        ...
        if (!DeviceUtils.hasVibrator(getActivity())) {
            PreferenceCategory volPanelCategory = (PreferenceCategory) findPreference("volume_panel_category");
            Preference v = findPreference("volume_dialog_haptic_feedback");
            if (v != null && volPanelCategory != null) {
                volPanelCategory.removePreference(v);
            }
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mHardwareKeysDisable) {
            ...
            return true;
        } else if (preference == mUseAxionVolumeDialog) {
            ...
            new BaseSearchIndexProvider(R.xml.infinity_suite_button) {
                ...
                public List<String> getNonIndexableKeys(Context context) {
                    ...
                    if (!DeviceUtils.hasVibrator(context)) {
                        keys.add("volume_dialog_haptic_feedback");
                        keys.add(KEY_VIBRATE_CATEGORY);
                        keys.add("vibrate_on_connect");
                        keys.add("vibrate_on_callwaiting");
                        keys.add("vibrate_on_disconnect");
                    }
